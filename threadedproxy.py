...
class myThread (threading.Thread):
    def __init__(self, threadID, s, data, q):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = "Thread" + threadID	
        self.q = q
        self.s = s
        self.data = data
    def run(self):
        print "Starting " + self.name		
        process_data(self.name, self.s, self.data, self.q)
        print "Exiting " + self.name
        
        

def process_data(threadName, s, data, inputs, q): 
...
def listen(server):
	inputs = [server]
	outputs = []
	m2tQueue = Queue.Queue(20) # master to thread queue
	t2mQueue = Queue.Queue(20) # thread to master queue
	threads = []
	threadID = 1
	while True:
		...
		for s in readable:
			if s is server:
				...
				thread = myThread(threadID, S, data)
				#t.daemon = True
				#t.start()				
				thread.start()
				...
				# close socket that select intercepted and that we were reading from
				s.close()
		while not t2mQueue.empty():
			queueLock.aquire()
			master_data = t2mQueue.get()
			queueLock.release()
